...
> **Maintained fork.** This is `@berrydev-ai/plugin-llm-wiki`, a fork of the LLM
> Wiki plugin from the [Paperclip monorepo](https://github.com/paperclipai/paperclip).
> It exists because the upstream `@paperclipai/plugin-llm-wiki@0.1.0` on npm is
> uninstallable — it ships a template named `.gitignore`, which npm renames to
> `.npmignore` on install, so the manifest throws `ENOENT` at load. See
> [UPSTREAM.md](UPSTREAM.md) for the fork policy and how to sync from upstream.

## Install

```bash
paperclipai plugin install @berrydev-ai/plugin-llm-wiki
```

The manifest still declares the plugin id `paperclipai.plugin-llm-wiki`, so an
instance that already had the upstream plugin installed keeps its wiki data and
applied migrations.

## Development

Requires Node 24 (see `.nvmrc`). Node 26 ships an experimental native
`localStorage` that shadows jsdom's and breaks the UI tests.
...
`npm test` includes `tests/packaging.spec.ts`, which packs the package, installs
the tarball for real, and loads the installed manifest — the guard against
reintroducing the packaging bug that motivated this fork.
